"""CLI entry point for the SharePoint DOCX -> Cloudflare R2 -> Neon pipeline.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Read DOCX files from SharePoint/OneDrive (or a local folder), extract profile "
            "details with Qwen2.5, upload the original DOCX to Cloudflare R2, and "
            "insert/update the profile in Neon."
        )
    )
    parser.add_argument("input", help="SharePoint/OneDrive shared folder URL, or a local DOCX file/folder")
    ...
    parser.add_argument("--limit", type=int, default=None, help="Process only the first N DOCX files")
    # Two by default: one record's R2 upload overlaps the other's model
    ...
    parser.add_argument("--retry-all", action="store_true",
                        help="Reprocess every discovered DOCX file, including previously processed ones")

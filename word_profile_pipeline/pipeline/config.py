"""Environment loading and runtime settings for the Word profile pipeline.
...
DEFAULT_ENV_VALUES = {
    ...
    # when two document workers are running.
...
def validate_env(*, dry_run: bool, use_llm: bool) -> None:
    ...
    if not dry_run:
        ...
        if not env_bool("STORAGE_ENABLED", True):
            raise SystemExit("ERROR: STORAGE_ENABLED must be true to upload DOCX files to Cloudflare R2.")

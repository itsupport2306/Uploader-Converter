...
Only DOCX files are discovered; each file is downloaded as bytes and stays a
DOCX for the whole pipeline. No conversion of any kind happens here.
"""
...
class DocxSource:
    """One DOCX found in the shared location (or on local disk)."""
...
def _walk(client: GraphClient, item: dict, *, parts: list[str], results: list[DocxSource], limit: int | None) -> None:
    ...
    if "file" in item:
        if Path(name).suffix.lower() in DOCX_EXTS:
            display_name = "/".join(current)
            results.append(DocxSource(
            ...
            if len(results) <= 5 or len(results) % 25 == 0:
                print(f"Discovered {len(results)} DOCX file(s)... latest: {display_name}")
        return
...
def iter_remote_docx(client: GraphClient, shared_url: str, limit: int | None) -> list[DocxSource]:
    root = _resolve_shared_item(client, shared_url)
    print("Listing DOCX files from the shared location...")
    results: list[DocxSource] = []
    _walk(client, root, parts=[], results=results, limit=limit)
    if not results:
        raise SystemExit(f"ERROR: no DOCX files found in shared location: {shared_url}")
    print(f"Finished listing. Found {len(results)} DOCX file(s).")
    return results


def iter_local_docx(input_path: Path, limit: int | None) -> list[DocxSource]:
    if input_path.is_file():
        if input_path.suffix.lower() not in DOCX_EXTS:
            raise SystemExit(f"ERROR: not a DOCX file: {input_path}")
        files = [input_path]
    elif input_path.is_dir():
        files = sorted(p for p in input_path.rglob("*") if p.is_file() and p.suffix.lower() in DOCX_EXTS)
    else:
        raise SystemExit(f"ERROR: input path not found: {input_path}")
    if not files:
        raise SystemExit(f"ERROR: no DOCX files found in {input_path}")
...
def download_docx_bytes(client: GraphClient, source: DocxSource) -> bytes:
    if not source.is_remote:
        raise ValueError("Remote download requested for a local DOCX source.")
    print(f"Downloading DOCX: {source.display_name}")
...
def read_docx_bytes(client: GraphClient | None, source: DocxSource) -> bytes:
    if source.is_remote:
        if client is None:
            raise RuntimeError("A Graph client is required to read remote DOCX files.")
        return download_docx_bytes(client, source)
    if source.local_path is None:
        raise RuntimeError("Local DOCX source is missing a path.")
